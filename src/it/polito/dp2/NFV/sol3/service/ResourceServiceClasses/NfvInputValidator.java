...
public class NfvInputValidator implements MessageBodyReader<JAXBElement<?>> {
	final String jaxbPackage = "it.polito.dp2.NFV.sol3.service.ServiceXML";
	Unmarshaller unmarshaller;
	Logger logger;
	String responseBodyTemplate;

	public NfvInputValidator() {
		logger = Logger.getLogger(NfvInputValidator.class.getName());

		try {			
			InputStream schemaStream2 = NfvInputValidator.class.getResourceAsStream("/xsd/DataTypeDeclaration.xsd");
			InputStream schemaStream = NfvInputValidator.class.getResourceAsStream("/xsd/NfvDeclaration.xsd");
            ...
            Schema schema = sf.newSchema(new Source[] {new StreamSource(schemaStream2), new StreamSource(schemaStream)});
            unmarshaller.setSchema(schema);
            
			InputStream templateStream = NfvInputValidator.class.getResourceAsStream("/html/BadRequestBodyTemplate.html");
            ...
            logger.log(Level.INFO, "Nfv validator initialized successfully");
